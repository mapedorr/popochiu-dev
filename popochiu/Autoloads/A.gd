extends "res://addons/Popochiu/Engine/AudioManager/AudioManager.gd"

# classes ----
const AudioCueSound := preload("res://addons/Popochiu/Engine/AudioManager/AudioCueSound.gd")
const AudioCueMusic := preload("res://addons/Popochiu/Engine/AudioManager/AudioCueMusic.gd")
# ---- classes

# cues ----
var mx_two_popochius: AudioCueMusic = preload("res://popochiu/Rooms/101/MxTwoPopochius.tres")
var sfx_toy_car: AudioCueSound = preload("res://popochiu/Rooms/101/SfxToyCar.tres")
# ---- cues
